...
# %%
methods = ["discriminative_ig", "discriminative_dl", "discriminative_ingrad"]
# %%
...
load_dir = "/nrs/funke/adjavond/projects/quac/synapses_v1/reports"
...
sns.displot(df, x="quac_scores", col="target_labels", row="labels", kde=True)
# %% Confusion matrices
from sklearn.metrics import confusion_matrix

sns.heatmap(
    confusion_matrix(df["labels"], df["pred_labels"], normalize="true"), annot=True
)
# %%
for name, sub_df in df.groupby("labels"):
    sns.heatmap(
        confusion_matrix(
            sub_df["target_labels"], sub_df["pred_target_labels"], normalize="true"
        ),
        annot=True,
    )
    plt.title(name)
...
def make_plot(idx):
    ...
    axes[0].imshow(
        x.squeeze()[xmin:xmax, ymin:ymax], alpha=alpha, cmap="gray", vmin=-1, vmax=1
    )
    axes[1].imshow(
        x_t.squeeze()[xmin:xmax, ymin:ymax], alpha=alpha, cmap="gray", vmin=-1, vmax=1
    )
