...
def do_the_thing(report, index, order, evaluator, save_dir, source, target):
    ...
    mask_cmap = LinearSegmentedColormap.from_list("mask_cmap", [colors[source], colors[target]])
    ...
    for mask_path, mask in zip(mask_paths, masks):
        ...
        plt.imsave(mask_path.replace(".png", "_gray.png"), mask, cmap="gray", vmin=0, vmax=1)
    ...
    plt.imsave(str(save_dir / f"{filename}_mask_optimal.png"), optimal_mask.squeeze(), vmin=0, vmax=1, cmap=mask_cmap)
    plt.imsave(str(save_dir / f"{filename}_mask_optimal_gray.png"), optimal_mask.squeeze(), vmin=0, vmax=1, cmap="gray")
    plt.imsave(str(save_dir / f"{filename}_hybrid_optimal.png"), optimal_hybrid.squeeze(), cmap='gray', vmin=0, vmax=255)
    ...
    plt.imsave(str(save_dir / f"{filename}_residual.png"), (image - counterfactual).squeeze(), cmap='coolwarm') # , vmin=-255, vmax=255)
    ...
    with open(save_dir / "classifications.csv", "a") as f:
        ...
        f.write(f"{filename}_real,{p_o[0]},{p_o[1]},{p_o[2]},{p_o[3]},{p_o[4]},{p_o[5]}\n")
        p_c = report.target_predictions[index]
        f.write(f"{filename}_counterfactual,{p_c[0]},{p_c[1]},{p_c[2]},{p_c[3]},{p_c[4]},{p_c[5]}\n")
        for hybrid_path, hybrid in zip(hybrid_paths, hybrids):
            p_h = evaluator.run_inference(normalize(hybrid))[0]
            f.write(f"{hybrid_path.stem},{p_h[0]},{p_h[1]},{p_h[2]},{p_h[3]},{p_h[4]},{p_h[5]}\n")
        p_h = evaluator.run_inference(normalize(optimal_hybrid))[0]
        f.write(f"{filename}_hybrid_optimal,{p_h[0]},{p_h[1]},{p_h[2]},{p_h[3]},{p_h[4]},{p_h[5]}\n")
